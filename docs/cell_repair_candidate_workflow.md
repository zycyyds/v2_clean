...
以下命令都在 `D:\mimic_graph\v2_clean-graph` 执行。可以统一使用 `graph-embedding`
环境，但调用 MiniMax 前必须在该环境安装 `agentscope==2.0.4.post1` 和 `httpx==0.28.1`，并通过
`python -m pip check`。也可以使用 `environment.yml` 创建独立环境；这只是依赖隔离选择，不是
独立 Agent 或方法要求。
